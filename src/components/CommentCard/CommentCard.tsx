...
import { CommentContent } from "app/(singles)/SingleContent";
...
const CommentCard: FC<CommentCardProps> = ({
  ...
  const { date, content, like, author, children } = comment;
  ...
  const renderCommentForm = (id: number) => {
    return (
      <SingleCommentForm
        threadOf={id}
        ...
        className="reply-comment-form flex-grow"
        value={replyCommentContent}
        onChange={(key, value) => onChangeReply?.(key, value)}
      />
    );
  };
  ...
  return (
    <>
      <div className={`nc-CommentCard ${className}`}>
        <div className="flex">
          <Avatar
          ...
          <div className="flex-grow flex flex-col p-4 ml-2 text-sm border border-neutral-200 rounded-xl sm:ml-3 sm:text-base dark:border-neutral-700">
            ...
            <div className="relative flex items-center pr-6">
              ...
              <Link
                ...
                href={`/author/${author?.href}` || AUTHOR.href}
              >
            ...
            {/* ACTION LIKE REPLY */}
            {isReplying ? (
              renderCommentForm(commentReplyId)
            ) : (
      ...
      {children && children.length > 0 && children.map((item) => (
        <div key={item.id} className="mt-3 ml-12">
          <CommentCard
